...
/**
 * AA-SchoolHealth Request Controller
 *
...
class Request extends CI_Controller {
    ...
    public function index()
    {
        ...
        $data ["subtitle"] = "AA-SchoolHealth - Admin Request";
        $data ["action"] = "auth/request/access_process";
    ...
    {
        // validation complete
        $response = $this->auth_model->admin_request();
        if ($response == NO_DUPLICATE_ADMIN)
        {
            ...
            // $this->email->cc($email);
            $this->email->subject("AA-SchoolHealth Request");
            $body = "
				...
				<tr>
				<td>Admin Request</td>
				<td>{$name} has requested access to AA-SchoolHealth Dashboard. <br/>Please " . anchor("" . base_url() . "login", "Login") . " to the dashboard to approve this request.</td>
				</tr>
				</table>
			";
        ...
        elseif ($response == DUPLICATE_REG)
        {
            ...
            $data ["error"] = "<div class=\"alert alert-danger alert-dismissable\">
                                	<button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button>
							 		User already registered. Contact the " . anchor("" . base_url() . "auth/request/registration_error?requestor=" . $this->input->post("first_name") . "
    ...
    {
        // set default attributes
        $data ["subtitle"] = "AA-SchoolHealth - Registration Error";
    ...
    function process_reg_error()
    {
        ...
        else
        {
            ...
            $this->auth_model->reg_error_message($data);
            // prepare email notification
            $this->email->set_newline("\r\n");
            $this->email->from("" . FROM_EMAIL . "", "" . FROM_NAME . "");
            $this->email->to("" . CC_EMAIL . "", "" . CC_NAME . "");
            $this->email->cc($from_email);
            $this->email->subject("AA-SchoolHealth - Registration Error");
            $body = "
				<table border=\"1\" style=\"border-width: thin; border-spacing: 2px; border-style: none; border-color: #ccc;\">
				<tr>
					<td>Requestor</td>
    ...
    function password_reset()
    {
        ...
        $data ["action"] = site_url("auth/request/password_reset_process");
        $data ["success"] = "";
        $data ["error"] = "";
        // load view
    ...
    function password_reset_process()
    {
        ...
        $acct = $this->auth_model->passreset($email)->row();
        if (!empty($acct))
        {
            ...
            // set user message
            $data ["success"] = "Password changed. Check your email...";
    ...
    protected function reset_password_allowed($user_id, $email)
    {
        ...
        // parse data array to update user table
        $this->auth_model->update_password($user_id, $acct);
        // prepare email notification
//        Default email send content
        ...
        $config['newline'] = "\r\n";
//        Default email send content end
