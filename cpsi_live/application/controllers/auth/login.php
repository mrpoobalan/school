...
//        exit;
        ...
        $username_count = $this->auth_model->check_username($this->input->post("username"));
...
// get user details
        ...
        } else if ($captcha_info == $captcha_value && $username_count['count'] == 0 || $username_count['status'] == 0) {
            $error_msg = "You are an unauthorized user";
            $this->index($error_msg);
        } else if ($captcha_info == $captcha_value) {
            $error_msg = "Invalid username or password";
            $this->index($error_msg);
        } else {
            $error_msg = "Captch was not correct";
            $this->index($error_msg);
        }
    }
...
//Get user Log Data
            ...
            if ($log_value['is_active'] == 0) {
                if ($login_success != AUTH_PASS) {
                    ...
                    //$reset_time = date("Y-m-d H:i:s", strtotime($loginTime) + 180);
...
//$endTime = $time_arr['endTime'];
                ...
                if ($log_count['loginTime'] == null) {
                    ...
                    //$end = date("Y-m-d H:i:s", strtotime($log_count['loginTime']) + 3);
                }
                ...
                if (($count % 5) == 0) {
                    ...
                    //$endTime = date("Y-m-d H:i:s", strtotime("+3 minutes"));
                } else {
                    $is_active = 0;
                    $endTime = '';
                }
