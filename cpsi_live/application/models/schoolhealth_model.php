...
//IF complted status // AND is_completed=1 added for latest appraisala n assessemnt wizard id get
        elseif (empty($search_opt['filter2']) && $search_opt['filter1_type'] == "wizard_status" && $search_opt['filter1'] == 45) {
            if ($user_role <= 20):
                $query = $this->db->query("SELECT * FROM form_sessions
                                       WHERE wizard_status=45 AND is_completed=1
...
//            exit('f');
            if ($user_role <= 20):
                ...
                /* $query = $this->db->query("SELECT *, MAX(`wizard_id`) AS wizard_id FROM (`form_sessions`)
                  ...
                  GROUP BY `unique_number` ORDER BY `unique_number` Desc");
                 * */
                $query = $this->db->query("SELECT *,MAX(`wizard_id`) AS wizard_id FROM (`form_sessions`) WHERE " . $search_opt['filter1_type'] . " LIKE '%" . $search_opt['filter1'] . "%' GROUP BY unique_number, wizard_sif_num ORDER BY  `wizard_id` DESC");
            else:
                $query = $this->db->query("SELECT *, MAX(`wizard_id`) AS wizard_id FROM (`form_sessions`)
...
//check_form_status
    ...
    public function check_assessment_rejected_comments($sif, $unum) {

        $sql = $this->db->select('user_id,nurseid,nursesupervisorid,rejectto')
                ->from('assessment_comments')
                ->where('sif', $sif)
                ->where('sif_unique_number', $unum)
                ->order_by('last_changed', 'DESC')
        ...
        if ($count >= 1):
            return $sql->row("user_id,nurseid,nursesupervisorid,rejectto");
        else:
            return FALSE;
        endif;
    }
