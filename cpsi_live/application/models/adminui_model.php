...
//        $nsid = $this->uri->segment(4);
    ...
    public function list_all_editpm() {
        $nsid = $this->uri->segment(4);
        if (!empty($_GET['term'])):
            $keyword = $_GET['term'];
        endif;
        ...
        return $this->db->query("SELECT * FROM admin_user
                                INNER JOIN user_role ON user_role.user_id = admin_user.user_id
                                WHERE user_role.role_id = 4 and admin_user.user_id <> '" . $nsid . "' and CONCAT(first_name,'',last_name) like '" . "%" . $keyword . "%" . "' ");
    }
    ...
    public function get_audit_update($limit = 10, $offset = 0, $wizardNumber, $orgdata) {
        ...
        if ($totcounts >= 1):
            $operation = "update";
            $sql = $this->db->select('audit_id,wizard_by,form_type,wizard_num,wizard_data,wizard_modified,wizard_sif_num')
                    ->from('form_sessions_audit')
                    ->where('wizard_num', $wnum)
                    ->where('operation', $operation)
                    ->where('unique_number', $sif_unum)
...
//        exit;
    ...
    public function get_latest_assessment($formtype) {
        $this->db->select("wizard_id,wizard_sif_num,unique_number,wizard_status,form_type")
                ->from('form_sessions')
                ->where('wizard_status', 45)
                ->where('form_type', 'Assessment')
                ->or_where('form_type', 'Appraisal')
                ->where('is_completed', '1')
                ->group_by("wizard_sif_num,wizard_id")
                ->order_by("complete_time DESC,wizard_id DESC,unique_number", 'Desc');

        $role = $this->db->get(0, 1);
        //echo $this->db->last_query();
        //exit;
        return ($role->num_rows() > 0) ? $role->result() : FALSE;
    }

    public function get_latest_appraisal($formtype) {
        $this->db->select("wizard_id,wizard_sif_num,unique_number,wizard_status,form_type")
                ->from('form_sessions')
                ->where('wizard_status', 45)
...
//		return ($this->db->affected_rows() == 1);
    ...
    function get_manageby_id($user_id) {
        $this->db->select("first_name, last_name, email_address,user_id");
        $this->db->where("user_id", $user_id);
     ...
     * Update Completed Date and Time 07/01/2016
     */

    public function approve($SIF) {
        $complete_time = date('Y-m-d H:i:s');
        $data = array('wizard_status' => COMPLETED, 'complete_time' => $complete_time);
        return $this->updateRecord($SIF, $data);
    }
    ...
    public function SaveComment($commentData, $status = '') {
        $unique_sif_number = $this->uri->segment(5);

        //Escalating the Comments
        $user_role = $this->get_user_role($this->session->userdata("user_id"));

        if ($user_role->role_id == 6) {
            $nurse = array('nurseid' => $user_role->user_id, 'rejectto' => 'Nurse');
        } else if ($user_role->role_id == 5 && $status == 'Escalate') {
            $nurse = array('nursesupervisorid' => $user_role->user_id, 'rejectto' => 'Nurse supervisor');
        } else if ($user_role->role_id == 5 && $status == '') {
            $nurse = array('nursesupervisorid' => $user_role->user_id, 'rejectto' => 'Nurse');
        } else {
            $nurse = array();
        }
        ...
        //Escalating the Comments to merge
        $commentTableData = array_merge($commentTableData, $nurse);
    ...
    public function LoadUserEditData($userID) {
        ...
        $sql = "SELECT au.first_name, au.last_name, au.status, au.username, au.email_address, r.role_id, r.name as roleName, r.description as roleDescription, um.manage_by
				...
				inner join role r on r.role_id = ur.role_id
				left join user_manager um on um.manage_id=au.user_id where au.user_id=? ORDER BY first_name,last_name ";
