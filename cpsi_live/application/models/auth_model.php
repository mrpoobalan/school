...
class Auth_model extends CI_Model {
    ...
    public function get_username_count($username) {
        $query = $this->db->query("select username from admin_user where username = '" . $username . "' and status=1 ");
    ...
    public function get_user_login_count($check_login_time, $usrid) {
        $query = $this->db->query("select * from user_log where loginTime > '" . $check_login_time . "' and loginSuccess = 0 and usr_id =  '" . $usrid . "' order by logId desc  ");
//        echo $this->db->last_query();
//        exit;
        $count = $query->num_rows();
        return $count;
    ...
    public function update_loginSuccess($user_id) {
        $query = $this->db->query("update user_log set loginSuccess = 2
     where usr_id = '{$user_id}' and loginSuccess = 0");
    }

    public function check_username($username) {
        $query = $this->db->query("select count(username) as count,status from admin_user where username = '{$username}'");
        return $query->row_array();
    }
