    ...
    public function get_user_management_chain($userID, $limitToUserID = null, $includeSelf = true, $limit = null, $offset = null, $isAdminUser = false) {
        ...
        if ($isAdminUser) {
            ...
            $sql = "select au.*,r.role_id, r.name as roleName, r.description as roleDescription
					...
					where status=1 and au.user_id in (
        ...
        //Order By ID
        $sql .= " order by au.first_name, au.last_name";
...
//        endif;
        ...
        if ($user_role == 50):
            $managedUserList = $managedUsers[1];
        endif;
        if ($user_role == 40) {
            $status = '15,35,25';
        }
...
//        exit;
    ...
    public function wizard_post($wizard_data, $wizard_num) {
        ...
        if ($query->row("wizard_num") == $wizard_num && $query->row("unique_number") == $unique_number) {
            ...
            //$wizard_data = array_merge($wizard_data, $version);
...
//Insert data for autosave
    ...
    public function create_assessment_form($sif, $uniquenumber, $user_level, $status = '') {
//echo $user_level . '<br>';
        if ($user_level <= 40) {
            if ($user_level == 40) {
                if ($status == 'rejected') {
                    // echo "here";
                    $sql = $this->db->query("SELECT nursesupervisorid as user_id  FROM assessment_comments WHERE sif = '" . $sif . "' AND sif_unique_number = '" . $uniquenumber . "'");
                    $count = $sql->num_rows();
                    if ($count >= 1):
                        $res = $sql->row_array();
                        $userid = $res['user_id'];
    ...
    public function get_log_details() {
        $sql = $this->db->query("select ul.logId,ul.usr_id,CONCAT(au.first_name,' ',au.last_name) as user_name,ul.loginTime,ul.ip_addr,ul.loginSuccess from user_log as ul Join admin_user au ON ul.usr_id=au.user_id ORDER BY ul.logId DESC");
        $count = $sql->num_rows();
        if ($count >= 1):
...
//Update firstname and last name in all the forms when click on goto final

    public function final_step_update($firstname, $lastname, $school, $sif, $unique_no) {
        $this->db->query("update form_sessions set first_name = '{$firstname}',last_name = '{$lastname}',
           student_school = '{$school}' where wizard_sif_num = '{$sif}' and form_type = 'assessment' and unique_number = '{$unique_no}'");
